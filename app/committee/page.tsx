...
export default function CommitteePage() {
  const [committeeMembers, setCommitteeMembers] = useState<Committee[]>([]);
  ...
  const handleCreateMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append("order", String(committeeMembers.length));
      if (formData.image) {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Failed to create committee member");
      }
      ...
      setError(
        error instanceof Error ? error.message : "Failed to create committee member",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleUpdateMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/committee/${editingMember._id}`, {
        method: "PATCH",
        body: formDataToSend,
      });
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.error || "Failed to update committee member");
      }
      ...
      setError(
        error instanceof Error ? error.message : "Failed to update committee member",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteMember = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.error || "Failed to delete committee member");
      }
      ...
      setError(
        error instanceof Error ? error.message : "Failed to delete committee member",
      );
      setTimeout(() => setError(null), 3000);
    }
  };

  // Reorder committee members function
  const reorderMembers = async (fromIndex: number, toIndex: number) => {
    if (fromIndex === toIndex) return;

    const newMembers = [...committeeMembers];
    const [draggedMember] = newMembers.splice(fromIndex, 1);
    newMembers.splice(toIndex, 0, draggedMember);
    ...
    try {
      const updates = updatedMembers.map((member, idx) => ({
        id: member._id,
        order: idx,
      }));

      const res = await fetch("/api/committee/reorder", {
        method: "POST",
        headers: {
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
    ...
    await reorderMembers(dragIndex, dropIndex);
  };
  ...
  const handleTouchMove = (e: React.TouchEvent, index: number) => {
    ...
    const targetCard = elements.find(el => el.hasAttribute?.('data-committee-index'));
    
    if (targetCard) {
      const targetIndex = parseInt(targetCard.getAttribute('data-committee-index') || '-1');
      if (targetIndex !== -1 && targetIndex !== touchStartIndex) {
        reorderMembers(touchStartIndex, targetIndex);
  ...
  const moveMemberUp = async (index: number) => {
    if (index > 0) {
      await reorderMembers(index, index - 1);
    }
  };

  const moveMemberDown = async (index: number) => {
    if (index < committeeMembers.length - 1) {
      await reorderMembers(index, index + 1);
    }
  };

  const handleEditMember = (member: Committee) => {
  ...
  const saveRearrangeOrder = async () => {
    try {
      const updates = committeeMembers.map((member, idx) => ({
        id: member._id,
        order: idx,
      }));
  ...
  // Group committee members by category
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-xl">Loading Committee Members...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-gray-300 text-sm sm:text-base mt-1 sm:mt-2">
                {isRearrangeMode
                  ? isMobile ? "Long press and drag to reorder, or use arrow buttons" : "Drag and drop to reorder committee members"
      ...
      <div className="container mx-auto px-4 sm:px-6 py-8 sm:py-12">
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-3 sm:px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm sm:text-base"
                    placeholder="Chairperson, University Name"
                  />
                </div>
        ...
        {/* Committee Sections by Category */}
        {Object.entries(groupedMembers).map(([category, members]) => (
          <div key={category} className="mb-12 last:mb-0">
            <div className="mb-6">
              <h2 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent inline-block">
                {category}
              </h2>
              <div className="w-20 h-1 bg-gradient-to-r from-purple-500 to-pink-500 mt-2 rounded-full"></div>
            </div>
            ...
            >
              {members.map((member, idx) => {
                ...
                const globalIndex = committeeMembers.findIndex(m => m._id === member._id);
                
                return (
                  <div
                    key={member._id}
                    data-committee-index={globalIndex}
                    draggable={!isMobile && isRearrangeMode}
                    onDragStart={(e) => !isMobile && isRearrangeMode && handleDragStart(e, globalIndex)}
                    onDragOver={(e) => !isMobile && isRearrangeMode && handleDragOver(e, globalIndex)}
                    onDragLeave={handleDragLeave}
                    onDrop={(e) => !isMobile && isRearrangeMode && handleDrop(e, globalIndex)}
                    onTouchStart={(e) => isMobile && handleTouchStart(e, globalIndex)}
                    onTouchMove={(e) => isMobile && handleTouchMove(e, globalIndex)}
                    ...
                    <div className="p-4 sm:p-5 md:p-6">
                      ...
                      {isRearrangeMode && isMobile && (
                        <div className="flex justify-between gap-2 mt-4 pt-3 border-t border-purple-500/20">
                          ...
                          <button
                            onClick={() => moveMemberDown(globalIndex)}
                            disabled={globalIndex === committeeMembers.length - 1}
                            className="flex-1 bg-blue-500/20 hover:bg-blue-500/30 disabled:opacity-30 disabled:cursor-not-allowed text-blue-400 py-2 rounded-lg flex items-center justify-center gap-2 transition-all"
                          >
        ...
        {committeeMembers.length === 0 && (
          <div className="text-center py-12 sm:py-20">
            <div className="text-gray-400 text-base sm:text-lg mb-4">No committee members found</div>
            <button
